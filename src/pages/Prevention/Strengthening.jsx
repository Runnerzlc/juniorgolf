...
import { strengtheningArray } from '../../assets/PreventionText';
...
const Strengthening = () => {
  ...
  const getItemUrls = async () => {
    const itemRefs = await storageRef.child('prevention/strengthening').listAll();
        ...
        <Row>
          {
            itemUrls.map((url, index) => {
              if (typeArray[index]) {
                return (
                  <Col className="player-wrapper" xs="10" md="6" lg="4" >
                    <ReactPlayer 
                      url={url}
                      controls
                    ...
                    <div className="stretching-title">
                      {strengtheningArray[index]}
                    </div>
                  </Col>
                )
              } else {
                return (
                  <Col className="stretching-image-wrapper" xs="10" md="6" lg="4">
                    <Image 
                      src={url}
